import { VARIABLE_ADD_SCOPES, CHEVRON_ICON_SVG_TEXT, SCOPE_ICON, isScopeIconSvg } from 'utils/common/constants';
...
const createScopeIcon = (scope, { muted = false } = {}) => {
  ...
  const scopeIcon = SCOPE_ICON[scope.type];
  if (scopeIcon && isScopeIconSvg(scopeIcon)) {
    icon.innerHTML = scopeIcon;
  } else {
    icon.textContent = scopeIcon || scope.label?.charAt(0)?.toUpperCase() || '?';
  }
